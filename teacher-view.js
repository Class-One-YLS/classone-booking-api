...
const API_BUILD = "2026.08.14-phase2-verification.1";
...
// A leave-covered half-hour that already had a booked regular/override slot (a real student's
// class) must keep that student's identity on the synthetic "teacher_leave" cell it produces here --
// previously this built a brand-new object that dropped studentName/studentId/type entirely, so any
// booked class caught by a teacher leave turned into a bare "Teacher on leave" cell on the Teacher
// View with no way to tell which student/class was affected, even though the admin's own Weekly
// Timetable (a different code path) still showed it correctly. Carrying the original slot's student
// fields forward, plus the class type, lets the Teacher View render both facts together: the class
// that was scheduled, and that it is affected by an approved leave (2026-09-12).
function teacherLeaveOffSlotFromCell(slot, leave) {
  ...
  return {
    ...
    affectedClassType: slot.locked ? (slot.type || "regular class") : "",
    unavailable: true,
...
function publicCellFromSlot(slot, teacher, state) {
  ...
  return {
    ...
    affectedClassType: slot.affectedClassType || "",
