...
async function normalizedRows(key = "production") {
  await ensureCoreTables();
  const sql = getSql();
  const [bookingRows, recurringRows, replacementRows, creditRows, activityRows, collectionRows, versionRows] = await Promise.all([
    ...
    // composed state (this query alone, "order by created_at desc limit 2500", was pulling the full
    // table since it has fewer than 2500 rows) -- the whole app was down. trimActivityLogUndoHistory()
    // below only strips .undo from the already-fetched, in-memory result; it never touched the raw
    // rows this table stores, so the stored data kept growing even though every *response* looked
    // trimmed. This fetches the newest UNDO_SNAPSHOT_RETENTION_COUNT rows with their undo data intact
    // (so "Undo" still works on recent entries) and every older row with `undo` stripped in SQL via
    // the jsonb `-` operator, before it ever leaves Postgres -- matching the exact same retention rule
    // as the in-memory trim, just moved earlier so a bloated table can never crash this query again.
    sql`
      (select data from activity_events_v2 where state_key = ${key} order by created_at desc limit ${UNDO_SNAPSHOT_RETENTION_COUNT})
      union all
      (select data - 'undo' as data from activity_events_v2 where state_key = ${key} order by created_at desc offset ${UNDO_SNAPSHOT_RETENTION_COUNT} limit ${2500 - UNDO_SNAPSHOT_RETENTION_COUNT})
    `,
