...
function recurringSourceSlotId(record) {
  ...
  return record && (record.recurringSourceSlotId || record.sourceSlotId || record.recurringScheduleId || record.movedFromRecurringClassId) || "";
}
...
function occurrenceKeyForSlot(slot, dateStr) {
  return occurrenceKeyFromParts(slot && (slot.id || slot.recurringScheduleId), dateStr);
}
...
function isDisplayableBookingRecord(booking) {
  return Boolean(booking) && (!isDeletedBooking(booking) || occurrenceResolutionStatus(booking) !== "deleted");
}
...
function inactiveBookingCanYieldToActiveSlot(booking) {
  return Boolean(booking && isHistoricalInactiveBookingRecord(booking) && (isSupersededBookingRecord(booking) || booking._wasSupersededForResolution === true));
}
...
function collectTeacherSlotsForDate(teacher, dateISO, occurrenceOutcomes = new Map()) {
  ...
  const regular = activeSlotRecords(teacher.regularSlots)
    ...
    .filter(slot => {
      const occurrence = occurrenceOutcomes.get(occurrenceKeyForSlot(slot, dateISO));
...
function isRegularClassType(type) {
  ...
  return !normalized || normalized === "regular class" || normalized === "fixed regular" || normalized === "fixed regular class";
}
...
function publicHolidayAffectsCell(cell, holiday) {
  ...
  if (cell.kind === "fixed" || cell.kind === "booking" || cell.locked || cell.studentName) {
    ...
    if (["cancelled", "student_not_show", "completed", "teacher_leave"].includes(status)) return false;
...
function publicCellFromBooking(booking, teacher, state) {
  const status = occurrenceResolutionStatus(booking) || normalizeClassStatus(booking) || booking.status || "booked";
  ...
  return {
    ...
    recurringScheduleId: booking.recurringScheduleId || booking.sourceSlotId || "",
...
function publicCellFromSlot(slot, teacher, state) {
  ...
  if (slot.reserved) {
    return {
      ...
      recurringScheduleId: slot.recurringScheduleId || slot.id || "",
      slotId: slot.id || "",
  ...
  datesBetween(from, to).forEach(dateISO => {
    ...
    times.forEach(time => {
      ...
      if (slot && slot.unavailable && (!booking || bookingAmendmentTime(slot) >= bookingAmendmentTime(booking))) {
        pushResolvedCell(publicCellFromSlot(slot, teacher, state));
        return;
      }
      if (booking && inactiveBookingCanYieldToActiveSlot(booking) && slotIsActiveOccurrence(slot)) {
        pushResolvedCell(publicCellFromSlot(slot, teacher, state));
        return;
      }
      if (booking) {
        if (isDeletedBooking(booking)) {
          if (isDisplayableBookingRecord(booking)) {
            pushResolvedCell(publicCellFromBooking(booking, teacher, state));
            return;
          }
          if (slot && (!slot.locked || slot.unavailable) && !slot.reserved) pushResolvedCell(publicCellFromSlot(slot, teacher, state));
          return;
        }
        pushResolvedCell(publicCellFromBooking(booking, teacher, state));
      } else if (slot) {
        ...
        if (leave) {
          pushResolvedCell(publicCellFromSlot(teacherLeaveOffSlotFromCell(slot, leave), teacher, state));
          return;
        }
        pushResolvedCell(publicCellFromSlot(slot, teacher, state));
      }
