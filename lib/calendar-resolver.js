...
function uniqueSlots(slots) {
  ...
  //
  // ADDED 2026-09-02: a narrower guard specifically for rank 1 ("plain open" -- not locked, not
  // unavailable, not an override; carries zero editorial intent, just "nothing recorded here"). A
  // rank-1 record must never mask a higher-rank record (a real booking, an explicit OFF, an
  // override) purely by having a newer updatedAt, and a higher-rank record must always be able to
  // replace a rank-1 record regardless of timestamp order. This is deliberately narrower than the
  // reverted rank-before-time rule above: it only ever fires when ONE side is rank 1, so it cannot
  // reproduce the 2026-08-28 regression (OFF is rank 5, LOCKED is rank 3/4 -- neither is rank 1;
  // OFF-vs-LOCKED and LOCKED-vs-LOCKED still resolve purely by "latest wins", untouched by this).
  // Root cause: a bulk backfill pass (2026-08-14, source "recurring_assignments_v2") re-touched
  // thousands of empty placeholder slots project-wide, bumping their updatedAt with no real content
  // change, which then silently outranked genuine July bookings by timestamp alone -- see Adelyneshia
  // / Chi Zhong Hyi Thursday 20:30 (booking real, admin UI showed the slot as empty/OFF).
  ...
  slots.forEach(slot => {
    ...
    if (!existing) { byTime.set(key, normalized); return; }
    const candidateRank = slotRank(normalized);
    const existingRank = slotRank(existing);
    if (candidateRank === 1 && existingRank > 1) return;
    if (existingRank === 1 && candidateRank > 1) { byTime.set(key, normalized); return; }
    ...
    if (newer || (sameTime && candidateRank > existingRank)) byTime.set(key, normalized);
  });
...
// both the admin Weekly Timetable and the read-only Teacher View -- with no indication a leave was
// ever approved for that time (2026-09-08, Tarry Pian Yee Sing: a whole-day leave correctly converted
// the two "Available" slots that existed that day to Teacher Leave, but every other half-hour showed
// as bare OFF, same as an ordinary non-working day). This enumerates every half-hour an active leave
// actually covers so the loop below can still emit a Teacher Leave cell even where no slot ever
// existed for that time.
  ...
  datesBetween(from, to).forEach(dateISO => {
    ...
    times.forEach(time => {
      ...
      if (booking) {
        ...
        // Neither a real slot nor a booking exists for this half-hour -- normally that just means an
        // ordinary non-working time (no cell at all, client renders it as blank/OFF). But if an active
        // teacher leave covers this exact time (see leaveTimesForDate above), surface it as a Teacher
        // Leave cell anyway so a whole-day leave reads as "on leave" for its full span, not just for
        ...
        if (leave) {
          ...
          pushResolvedCell(publicCellFromSlot(teacherLeaveOffSlotFromCell(syntheticSlot, leave), teacher, state, displayNowMs));
        }
      }
    });
  });
