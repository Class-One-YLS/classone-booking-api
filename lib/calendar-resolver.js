...
function uniqueSlots(slots) {
  // Rank takes priority over timestamp: a locked class with a student (rank 4) can only ever
  // be superseded by an equal-or-higher rank record (another locked+student assignment, or an
  // explicit unavailable/off record), never by a lower-rank "open" placeholder just because that
  // placeholder happens to carry a newer updatedAt. Timestamp is only the tiebreaker within the
  // same rank. Without this, an unrelated later write (releasing an old reservation, a bulk data
  // migration, etc.) that happens to touch the same teacher/day/time cell can silently hide an
  // already-booked regular class from Weekly Timetable and Teacher View. See incident 2026-08-25
  // (Saw Yik Xuan / Hoh Kar Yee Tuesday 18:00, plus similar hidden classes found for Adelyneshia
  // and Lee Shok Yuong) for the real-world case this guards against.
