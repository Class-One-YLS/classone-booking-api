...
function isPureDeletedOccurrenceRecord(booking) {
  ...
  return booking.deleted === true || booking.archived === true || booking.active === false || normalizeClassStatus(booking) === "deleted";
}
...
function inactiveBookingCanYieldToActiveSlot(booking, slot) {
  ...
  const slotOccurrenceKey = occurrenceKeyForSlot(slot, dateOnly(booking.date || slot.date));
  return Boolean(!bookingOccurrenceKey || bookingOccurrenceKey !== slotOccurrenceKey);
}
