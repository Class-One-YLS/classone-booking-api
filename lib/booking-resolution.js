function parsedTime(value) {
  const time = Date.parse(value || "");
  return Number.isFinite(time) ? time : 0;
}

function bookingAmendmentTime(booking) {
  const values = [
    booking && booking.slotRevisionAt,
    booking && booking.statusChangedAt,
    booking && booking.updatedAt,
    booking && booking.updated_at,
    booking && booking.changedAt,
    booking && booking.changedSlot && booking.changedSlot.changedAt,
    booking && booking.rebookedAt,
    booking && booking.cancelledAt,
    booking && booking.completedAt,
    booking && booking.studentNotShowAt,
    booking && booking.supersededAt,
    booking && booking.deletedAt,
    booking && booking.createdAt,
    booking && booking.created_at
  ].map(parsedTime).filter(Boolean);
  return values.length ? Math.max(...values) : 0;
}

function normalizeClassStatus(record) {
  const values = typeof record === "string"
    ? [record]
    : [
        record && record.outcome,
        record && record.finalStatus,
        record && record.classOutcome,
        record && record.outcomeStatus,
        record && record.resolutionStatus,
        record && record.resolutionOutcome,
        record && record.businessStatus,
        record && record.status
      ];
  for (const value of values) {
    const normalized = String(value || "").trim().toLowerCase().replace(/\s+/g, "_").replace(/-/g, "_");
    if (!normalized) continue;
    if (["cancel", "cancelled", "canceled"].includes(normalized)) return "cancelled";
    if (["student_not_show", "student_no_show", "not_show", "no_show"].includes(normalized)) return "student_not_show";
    if (["teacher_leave", "teacher_on_leave"].includes(normalized)) return "teacher_leave";
    if (["public_holiday", "holiday"].includes(normalized)) return "public_holiday";
    if (["completed", "complete", "passed", "finalized", "finalised", "finished"].includes(normalized)) return "completed";
    if (["booked", "booking"].includes(normalized)) return "booked";
    if (["available"].includes(normalized)) return "available";
    if (["off", "unavailable"].includes(normalized)) return "off";
    if (["deleted", "removed"].includes(normalized)) return "deleted";
  }
  return "";
}

function isDeletedBooking(booking) {
  if (!booking) return true;
  if (booking.deleted === true || String(booking.status || "").toLowerCase() === "deleted") return true;
  const classStatus = normalizeClassStatus(booking);
  if (classStatus && classStatus !== "deleted") return false;
  return classStatus === "deleted";
}

function isSupersededBooking(booking) {
  return Boolean(booking && (booking.supersededByBookingId || booking.rebookedByBookingId || booking.superseded === true));
}

function bookingTieRank(booking) {
  const status = normalizeClassStatus(booking) || "booked";
  if (status === "deleted") return -100;
  if (status === "student_not_show") return 7;
  if (["cancelled", "public_holiday", "teacher_leave"].includes(status)) return 6;
  if (status === "booked") return 5;
  if (status === "completed") return 4;
  return 1;
}

function isTerminalNonCompletedStatus(status) {
  return ["cancelled", "student_not_show", "teacher_leave", "public_holiday"].includes(status);
}

function isHistoricalInactiveBooking(booking) {
  const status = normalizeClassStatus(booking) || String(booking && booking.status || "").toLowerCase();
  return ["cancelled", "public_holiday", "teacher_leave", "off", "available"].includes(status);
}

function dateOnly(value) {
  if (!value) return "";
  const text = String(value);
  const match = text.match(/\d{4}-\d{2}-\d{2}/);
  return match ? match[0] : text.slice(0, 10);
}

function recurringSourceSlotId(booking) {
  if (!booking) return "";
  const keySource = recurringSourceFromOccurrenceKey(booking);
  return booking.normalizedRecurringAssignmentId ||
    booking.assignmentId ||
    booking.recurringAssignmentId ||
    booking.recurringSourceSlotId ||
    booking.sourceSlotId ||
    booking.recurringScheduleId ||
    booking.movedFromRecurringClassId ||
    keySource ||
    "";
}

function recurringSourceFromOccurrenceKey(booking) {
  const values = [
    booking && booking.sourceOccurrenceKey,
    booking && booking.occurrenceKey,
    String(booking && booking.sourceOccurrenceId || "").startsWith("occurrence:") ? String(booking.sourceOccurrenceId).slice("occurrence:".length) : "",
    String(booking && booking.occurrenceId || "").startsWith("occurrence:") ? String(booking.occurrenceId).slice("occurrence:".length) : ""
  ];
  for (const value of values) {
    const text = String(value || "").trim();
    if (!text || !text.includes("|")) continue;
    const [source, date] = text.split("|");
    if (source && dateOnly(date)) return source;
  }
  return "";
}

function occurrenceIdentity(booking) {
  if (!booking) return "";
  let key = booking.sourceOccurrenceKey || booking.occurrenceKey || "";
  if (!key && String(booking.sourceOccurrenceId || "").startsWith("occurrence:")) key = String(booking.sourceOccurrenceId).slice("occurrence:".length);
  if (!key && String(booking.occurrenceId || "").startsWith("occurrence:")) key = String(booking.occurrenceId).slice("occurrence:".length);
  if (!key) {
    const sourceSlotId = recurringSourceSlotId(booking);
    const date = dateOnly(booking.date || booking.occurrenceDate);
    if (sourceSlotId && date) key = `${sourceSlotId}|${date}`;
  }
  return String(key || "");
}

function sameOccurrence(candidate, existing) {
  const candidateOccurrence = occurrenceIdentity(candidate);
  const existingOccurrence = occurrenceIdentity(existing);
  if (candidateOccurrence && existingOccurrence) return candidateOccurrence === existingOccurrence;
  const candidateName = String(candidate && candidate.studentName || "").trim().toLowerCase();
  const existingName = String(existing && existing.studentName || "").trim().toLowerCase();
  const sameStudent = Boolean(
    (candidate && existing && candidate.studentId && existing.studentId && candidate.studentId === existing.studentId) ||
    (candidateName && candidateName === existingName)
  );
  return Boolean(
    candidate && existing &&
    candidate.teacherId && existing.teacherId && candidate.teacherId === existing.teacherId &&
    dateOnly(candidate.date) && dateOnly(candidate.date) === dateOnly(existing.date) &&
    String(candidate.time || "") === String(existing.time || "") &&
    sameStudent
  );
}

function bookingChoice(candidate, existing) {
  if (!existing) return { useCandidate: true, reason: "first active record" };
  const sameRecord = Boolean(candidate && existing && candidate.id && existing.id && candidate.id === existing.id);
  if (!sameRecord && !sameOccurrence(candidate, existing)) {
    const candidateInactive = isHistoricalInactiveBooking(candidate);
    const existingInactive = isHistoricalInactiveBooking(existing);
    if (candidateInactive !== existingInactive) {
      return {
        useCandidate: !candidateInactive,
        reason: candidateInactive ? "historical inactive record loses to active occurrence" : "active occurrence beats historical inactive record"
      };
    }
  }
  if (sameRecord || sameOccurrence(candidate, existing)) {
    const candidateStatus = normalizeClassStatus(candidate);
    const existingStatus = normalizeClassStatus(existing);
    if (isTerminalNonCompletedStatus(candidateStatus) && existingStatus === "completed") {
      return {
        useCandidate: true,
        reason: "terminal non-completed outcome beats generated completion for same occurrence"
      };
    }
    if (candidateStatus === "completed" && isTerminalNonCompletedStatus(existingStatus)) {
      return {
        useCandidate: false,
        reason: "generated completion loses to terminal non-completed outcome for same occurrence"
      };
    }
  }
  const candidateTime = bookingAmendmentTime(candidate);
  const existingTime = bookingAmendmentTime(existing);
  if (candidateTime !== existingTime) {
    return {
      useCandidate: candidateTime > existingTime,
      reason: candidateTime > existingTime ? "newer amendment time" : "older amendment time"
    };
  }
  const candidateRank = bookingTieRank(candidate);
  const existingRank = bookingTieRank(existing);
  if (candidateRank !== existingRank) {
    return {
      useCandidate: candidateRank > existingRank,
      reason: candidateRank > existingRank ? "equal timestamp status tie-break" : "lost equal timestamp status tie-break"
    };
  }
  const useCandidate = String(candidate.id || "") > String(existing.id || "");
  return { useCandidate, reason: useCandidate ? "equal timestamp stable id tie-break" : "lost stable id tie-break" };
}

function resolveBookingRecords(bookings, keyFor) {
  const winners = new Map();
  const traces = new Map();
  (Array.isArray(bookings) ? bookings : []).forEach(booking => {
    const key = keyFor(booking);
    if (!key) return;
    const trace = traces.get(key) || [];
    if (isSupersededBooking(booking)) {
      trace.push({ booking, selected: false, reason: `superseded by ${booking.supersededByBookingId || "newer booking"}` });
      traces.set(key, trace);
      return;
    }
    const existing = winners.get(key);
    const choice = bookingChoice(booking, existing);
    trace.push({ booking, selected: choice.useCandidate, reason: isDeletedBooking(booking) && choice.useCandidate ? "newer deletion tombstone wins" : choice.reason });
    if (choice.useCandidate) winners.set(key, booking);
    traces.set(key, trace);
  });
  return { winners, traces };
}

module.exports = {
  bookingAmendmentTime,
  bookingChoice,
  bookingTieRank,
  isDeletedBooking,
  isSupersededBooking,
  normalizeClassStatus,
  resolveBookingRecords
};
